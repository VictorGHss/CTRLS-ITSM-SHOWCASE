...
public class MonitorAppointmentNudgesUseCase {
    ...
    public void execute() {
        ...
        if (nowTime.isBefore(java.time.LocalTime.of(7, 0)) || nowTime.isAfter(java.time.LocalTime.of(19, 0))) {
            log.info("[NUDGE-MONITOR] Horário atual ({}) fora da janela de atendimento da organização (07:00 às 19:00). Abortando ciclo.", nowTime);
            return;
        }
                ...
                if (lockedSession.getFeegowAppointmentId() != null && !lockedSession.getFeegowAppointmentId().isBlank()) {
                    try {
                        ...
                        if (feegowAppt != null) {
                            ...
                            // Se a data/hora da consulta tiver mudado no Feegow (reagendamento efetuado internamente)
